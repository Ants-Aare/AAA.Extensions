...
namespace AAA.Extensions
{
    public static class TypeExtension
    {
        ...
        public static string ToCompilableString(this Type type)
        {
            ...
            {
                var values = ((IEnumerable<Type>)type.GetGenericArguments()).Select<Type, string>((Func<Type, string>)(argType => argType.ToCompilableString()));
        ...
        {
            var typeString1 = TypeExtension.GenerateTypeString(typeString);
        ...
        private static string GenerateGenericArguments(string typeString)
        {
            ...
            {
                var typeString1 = TypeExtension.GenerateTypeString(match.Groups["arg"].Value);
        ...
        private static string GenerateArray(string typeString)
        {
            typeString = Regex.Replace(typeString, "(?<type>[^\\[]*)(?<rank>\\[,*\\])",
                (MatchEvaluator)(match => TypeExtension.GenerateTypeString(match.Groups["type"].Value) + match.Groups["rank"].Value));
            return typeString;
        }
    }
}